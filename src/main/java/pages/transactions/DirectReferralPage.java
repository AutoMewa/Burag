...
public class DirectReferralPage extends TransactionsNavigationPanelComponent {
    ...
    private By referralSaveConfirmationModal = By.xpath("//div[contains(@class, 'jconfirm') and contains(@class, 'white')]");
    ...
    public MyTransactionsPage addNewReferral(String orgUnitNumber, String actionType, String deliveryMethod) {
        driver.element().click(departmentNumber).clear(departmentNumber).type(departmentNumber, orgUnitNumber)
                .waitToBeReady(orgUnitAutoCompleteMenu).click(firstOrgChartAutoSuggestion);
        driver.element().select(referralAction, actionType).select(referralDeliveryMethod, deliveryMethod);
        driver.element().click(referralFollowUpCheckBox);
        driver.element().click(referralSendButton)
                .waitToBeReady(referralSaveConfirmationModal)
