...
public class OutTransactionDraftPage {
    ...
    private By incomDestinationNumber = By.xpath("//input[contains(@class, 'InboundDepNum') and contains(@class, 'txtDepartmentNumber')]");
    private By outDestinationName = By.xpath("//input[contains(@class, 'InboundDepName') and contains(@class, 'txtDepartmentName')]");
    private By outDestinationFilter = By.xpath("//span[contains(@class, 'input-group-text') and contains(@class, 'hyperlink')]");
    private By outDestinationModal = By.id("modalBody__dvDestinationTree");
    private By outDestinationEntry = By.xpath("//div[contains(text(),\"" + testData.getTestData("outgoingDestination") + "\") and contains(@data-name,'DestinationId')]");
    ...
    private By uploadNewAttachmentButton = By.xpath("//a[contains(@class,'btn-upload') and @href='#']");
    ...
    private By printTrxDeliveryReportButton = By.xpath("//a[contains(@onclick,'PrintTransactionDeliveryReport')]");
    ...
    private By printConfirmationModal = By.xpath("//div[contains(@class, 'jconfirm') and contains(@class, 'white')]");
    ...
    private By confirmationDialogue = Locator.hasTagName("div")
            .containsText("هل أنت متأكد من عملية التعديل؟").insideShadowDom(confirmationAndSuccessModal).build();
    ...
    private By transactionNumberFromConfirmation = By.xpath("//div[contains(@class,'col-xs-4')]/span[contains(@class,'indent-text')]");
    private By backToIncomTransactionButton = By.xpath("//button[contains(@class, 'redirect-to-tray')]");
    private By printTransactionDeliveryReportFromPopup = By.xpath("//button[contains(@class,'move-to-assignment')]");
    ...
    public OutTransactionDraftPage modifyTransactionSubject() {
        driver.element().type(subjectTextField, "Description: " + GeneralOperations.getCurrentDateTime("yyyy-MM-dd HH:mm:ss"));
    ...
    @Step("اضافة ملحقات")
    public void addAttachmentToTransaction(String transactionAttachmentType, String transactionAttachmentLocation) {
        driver.element().click(attachFileButton).select(attachmentType, transactionAttachmentType).captureScreenshot(attachmentType);
    ...
    public OutTransactionDraftPage addGeneralTransaction() {
        ...
        driver.element().click(outDestinationEntry)
                .verifyThat(outDestinationName).text().contains(testData.getTestData("outgoingDestination"));
        driver.element().type(subjectTextField, "Description: " + GeneralOperations.getCurrentDateTime("yyyy-MM-dd HH:mm:ss"));
